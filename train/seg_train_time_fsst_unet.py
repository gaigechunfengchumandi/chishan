...
from models.model_unet import UNet, ECGDataset
...
def main():
    ...
    np.random.seed(42)
    # 配置参数
    data_dir = '/Users/xingyulu/Public/afafaf/房颤与非房颤/用2例数据尝试训练'
    batch_size = 4 # 批次大小
    ...
    # 创建数据集和数据加载器
    train_dataset = ECGDataset(train_dir,'time') 
    val_dataset = ECGDataset(val_dir,'time')
    test_dataset = ECGDataset(test_dir,'time')
    ...
    # 初始化模型
    model = UNet(input_size=2500, use_transformer=True)
